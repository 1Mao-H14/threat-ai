...
class ActionEngine:
    ...
    # ── MANUAL BLOCK / UNBLOCK (called by the dashboard) ───────────────────
    def set_account_enabled(self, user: str, enabled: bool) -> dict:
        """
        Enable (unblock) or disable (block) an account on demand from the
        dashboard. Returns {"success": bool, "detail": str}. Best-effort:
        if Entra ID / Graph is unreachable the dashboard still records the
        intended state locally so the UI stays consistent.
        """
        uid = self._get_user_id(user)
        if not uid:
            return {"success": False,
                    "detail": "Entra ID user id not found or Graph unavailable"}
        try:
            r = requests.patch(
                f"{self.GRAPH}/users/{uid}",
            ...
            if r.status_code == 204:
                verb = "UNBLOCKED" if enabled else "BLOCKED"
                logger.warning(f"[{user}] account {verb} via dashboard")
                return {"success": True,
                        "detail": f"Compte {'réactivé' if enabled else 'bloqué'} (Azure AD HTTP 204)"}
            return {"success": False, "detail": f"Graph HTTP {r.status_code}: {r.text}"}
        except Exception as e:
            logger.error(f"[{user}] set_account_enabled error: {e}")
